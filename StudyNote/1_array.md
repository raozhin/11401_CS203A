...
[cite_start]陣列 (Array) 是一種將**相同資料型態 (Data Type)** 的元素，儲存在**連續記憶體空間 (Contiguous Memory)** 中的資料結構 [cite: 3-6]。

### 關鍵特性
* [cite_start]**隨機存取 (Random Access)**：透過索引 (Index) 可以直接計算出記憶體位址，存取時間複雜度為 $O(1)$ [cite: 5, 459-468]。
* [cite_start]**固定大小 (對於靜態陣列)**：宣告時需決定大小，無法隨意擴充 (除非使用動態陣列) [cite: 5, 147]。
...
[cite_start]陣列作為一個 ADT，定義了一組 `<index, value>` 的配對集合，並支援以下操作 [cite: 122-139, 966-974]：
...
[cite_start]陣列在記憶體中的表現形式取決於它是「靜態」還是「動態」[cite: 142-153]。

### 3.1 靜態陣列 (Static Array)
[cite_start]記憶體配置在 **Stack** (堆疊) 上，大小在編譯時期 (Compile-time) 決定 [cite: 143-147]。
...
* **Insert at index **: 需要將  之後的所有元素向**後**移 ()。最壞情況 。
* **Delete at index **: 需要將  之後的所有元素向**前**移 ()。最壞情況 。

4.2 搜尋 (Searching) 

* **線性搜尋 (Linear Search)**: 適用於未排序陣列。從頭走到尾。時間複雜度 。
* **二元搜尋 (Binary Search)**: 適用於**已排序**陣列。每次切一半查找。時間複雜度 。

4.3 排序 (Sorting) 

雖然現代語言多直接呼叫 `qsort` 或 `std::sort`，但理解基礎排序對陣列操作很有幫助：

1. **氣泡排序 (Bubble Sort)**: 兩兩比較交換，最大值浮到最後。。
2. **選擇排序 (Selection Sort)**: 每一輪找出最小值，放到前面。。
3. **插入排序 (Insertion Sort)**: 像撲克牌理牌，將元素插入已排序區塊的正確位置。。
...
## 7. 補充進階觀念 (Advanced Concepts)

### 7.1 擴充策略 (Growth Strategy)

動態陣列 (如 C++ `std::vector`) 在容量滿時，通常會以 **倍數 (如 2 倍)** 擴充。若每次只增加 1 個空間，插入  個元素的總成本會變成 ；若倍數增長，均攤 (Amortized) 後每次插入成本接近 。

### 7.2 安全性問題：緩衝區溢位 (Buffer Overflow)

這是 C 語言陣列最著名的漏洞。

* **問題**：存取 `arr[10]` 但陣列大小只有 10 (合法索引 0-9)。
* **後果**：可能會覆蓋相鄰變數的值、Return Address，導致程式崩潰或安全漏洞。
* **防禦**：使用 `std::vector` 的 `.at()` 方法，或在迴圈中嚴格檢查邊界。
